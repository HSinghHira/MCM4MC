...
public class OreESP extends Module {
    ...
    private BigCavesESP bigCavesESP;
    ...
    public OreESP() {
        super(MCM4MC.JH_RENDER_CAT, "ore-ESP", "Finds and highlights exposed ores (optimized).");
    ...
    public void onActivate() {
        bigCavesESP = Modules.get().get(BigCavesESP.class);
        exposedOres.clear();
    ...
    private void scanBigCaves() {
        if (bigCavesESP == null || !bigCavesESP.isActive() || bigCavesESP.foundCaves.isEmpty()) return;

        for (BlockPos caveAreaPos : bigCavesESP.foundCaves) {
    ...
    private void scanAreaForExposedOres(BlockPos areaStartPos) {
        ...
        int volumeSize = bigCavesESP.checkVolumeSize.get();
