...
import im.hira.tweaks.gui.screens.KeywordsScreen;
import im.hira.tweaks.util.Keyword;
...
public class ChatUtility extends Module {
    ...
    public ChatUtility() {
        super(MCM4MC.JH_UTILITIES_CAT, "chat-utility", "Various chat-related utilities.");
    }
    ...
    private void setupTrayIcon() {
        ...
        try (InputStream is = MCM4MC.class.getResourceAsStream("/assets/jatthira/icon.png")) {
            if (is == null) {
                MCM4MC.LOG.warn("Could not find icon for toast notification.");
                return;
            }
            BufferedImage image = ImageIO.read(is);
            if (image == null) {
                MCM4MC.LOG.warn("Failed to read icon for toast notification.");
                return;
            }
            Image scaledImage = image.getScaledInstance(16, 16, Image.SCALE_SMOOTH);
            trayIcon = new TrayIcon(scaledImage, "Jatt Hira");
            trayIcon.setImageAutoSize(true);
            try {
                SystemTray.getSystemTray().add(trayIcon);
            } catch (AWTException e) {
                MCM4MC.LOG.error("Failed to add system tray icon.", e);
            }
        } catch (Exception e) {
            MCM4MC.LOG.error("Failed to set up system tray icon.", e);
        }
    }
