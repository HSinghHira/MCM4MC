...
public class MessageLagger extends Module {
    ...
    public MessageLagger() {
        super(MCM4MC.JH_CRASH_CAT, "Messages Lagger", "Sends dense messages that lag other players on the server.");
    }
    ...
    private void sendLagWhisper() {
        ...
        if (players.isEmpty()) return; // Avoid crash if no players are present
        PlayerEntity player = players.get(new Random().nextInt(players.size()));
        String message = generateLagMessage();
        mc.player.networkHandler.sendChatCommand("msg " + player.getGameProfile().getName() + " " + message);
    }
