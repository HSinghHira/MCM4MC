...
public class WMeteorWindow extends WWindow implements MeteorWidget {
    ...
    protected void onRender(GuiRenderer renderer, double mouseX, double mouseY, double delta) {
        if (expanded || animProgress > 0) {
            GuiUtils.quadRounded(renderer,x , y + header.height / 2, width, height - header.height / 2, theme().backgroundColor.get(), ((Starlight)theme).roundAmount(), false);
        }
    }
    ...
    private class WMeteorHeader extends WHeader {
        ...
        @Override
        protected void onRender(GuiRenderer renderer, double mouseX, double mouseY, double delta) {
            GuiUtils.quadRounded(renderer, this, theme().accentColor.get(), ((Starlight)theme).roundAmount());
        }
    }
}
