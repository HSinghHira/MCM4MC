...
public class WMeteorTopBar extends WTopBar implements MeteorWidget {
    ...
    public void init() {
        for (Tab tab : Tabs.get()) {
            add(new im.hira.tweaks.gui.themes.StarLight.widgets.WMeteorTopBar.WTopBarButton(tab));
        }
    }

    protected int getState(im.hira.tweaks.gui.themes.StarLight.widgets.WMeteorTopBar.WTopBarButton btn) {
        int a = 0;
    ...
    protected class WTopBarButton extends WPressable {
        ...
        protected void onRender(GuiRenderer renderer, double mouseX, double mouseY, double delta) {
            ...
            switch (getState(this)) {
                case 1:
                    GuiUtils.quadRoundedSide(renderer, this, color, ((Starlight)theme).roundAmount(), false);
                    break;
                case 2:
                    GuiUtils.quadRoundedSide(renderer, this, color, ((Starlight)theme).roundAmount(), true);
                    break;
                case 3:
                    GuiUtils.quadRounded(renderer, this, color, ((Starlight)theme).roundAmount());
                    break;
                default:
